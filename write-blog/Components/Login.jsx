...
export default function Login(props) {
  function loginUser(formData) {
    ...
    fetch("http://backend-production-acfb.up.railway.app:5000/users/login", {
      method: "POST",
      headers: {
      ...
      .then(async (res) => {
        ...
        if (!data.user.isAuthor) {
          props.setErrorInView("Access Denied - Not an Author");
          props.setCurrentError(true);
          return;
        }
    ...
    <>
      ...
      <div className="login-form">
        <form action={loginUser}>
          ...
          <div>Username: author | Password: test</div>
        </form>
      </div>
    </>
  );
